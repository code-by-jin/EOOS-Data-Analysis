...
def end_condition(df, curr_index, th_feces_deriv=0.001, th_urine_deriv=0.001, th_end_last=15):
    th_last_index = min(curr_index+th_end_last, df.index[-1])
    con_feces = (df.loc[curr_index+1:th_last_index, 'feces_deriv'] <= th_feces_deriv).all()
    con_urine = (df.loc[curr_index+1:th_last_index, 'urine_deriv'] <= th_urine_deriv).all()
...
def detect_event(df, path_date, th_feces_deriv=0.001, th_urine_deriv=0.001, 
